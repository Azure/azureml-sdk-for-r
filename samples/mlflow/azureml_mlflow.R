...
get_tracking_uri <- function(region, subscription_id, resource_group, workspace_name)
{
    sprintf(paste0('https://%s.experiments.azureml.net/history/v1.0/subscriptions/%s/resourceGroups/%s/providers/',
                    'Microsoft.MachineLearningServices/workspaces/%s'), region, subscription_id, resource_group,
                    workspace_name)
}
...
# client_id is service principal's application (client) id. password is service principal's secret
...
token <- get_azure_token(c('https://management.azure.com/.default','offline_access'), tenant = tenant_id,
                        app = client_id, password = secret, version=2)

Sys.setenv('MLFLOW_TOKEN' = token$credentials$access_token)

# set up background refresh of token if this has to run for a long time in a script
...
tracking_uri <- get_tracking_uri(region, subscription_id, resource_group, workspace_name)
...
# hitting errors on windows running the following. Other people have also reported the following:
