...
#' to start training.
#' @param script_params A named list of the command-line arguments to pass to the 
#' training script specified in `entry_script`.
...
#' environment. If `FALSE`, a CPU-based image will be used. Default Docker images
#' (CPU or GPU) will only be used if the `custom_docker_image` parameter is not
#' set.
...
#' @param max_run_duration_seconds An integer of the maximum allowed time for the run. 
#' Azure ML will attempt to automatically cancel the run if it takes longer 
#' than this value.
#' @param environment The `Environment` object that configures the R environment where 
#' the experiment is executed. This parameter is mutually exclusive with the other 
#' environment-related parameters `custom_docker_image`, `image_registry_details`, 
#' `use_gpu`, `environment_variables`, `shm_size`, `cran_packages`, `github_packages`, and 
#' `custom_url_packages` and if set will take precedence over those parameters.
