...
#' should attempt to scale the web service. Defaults to `1`.
...
#' scoring calls to the web service. Defaults to `60000`.
#' @param replica_max_concurrent_requests An int of the number of maximum concurrent
#' requests per node to allow for the web service. Defaults to `1`.
#' @param max_request_wait_time An int of the maximum amount of time a request 
#' will stay in the queue (in milliseconds) before returning a 503 error. Defaults 
#' to `500`.
...
#' @param token_auth_enabled If `TRUE`, enable token-based authentication for the web service. 
#' If enabled, users can access the web service by fetching an access token using their Azure 
#' Active Directory credentials. Both `token_auth_enabled` and `auth_enabled` cannot be set to 
#' `TRUE`.
#' @export
#' @md
