...
#' 
#' @description
#' Provision Azure Machine Learning Compute (AmlCompute) as a compute target for 
#' training. AmlCompute is a managed-compute infrastructure that allows the user 
#' to easily create a single or multi-node compute. To create a persistent 
#' AmlCompute resource that can be reused across jobs, make sure to specify the 
#' `vm_size` and `max_nodes` parameters. The compute can then be shared with other 
#' users in the workspace and is kept between jobs. If `min_nodes = 0`, the compute 
#' autoscales down to zero nodes when it isn't used, and scales up automatically 
#' when a job is submitted.
#' 
#' AmlCompute has default limits, such as the number of cores that can be allocated. 
#' For more information, see [Manage and request quotas for Azure resources](https://docs.microsoft.com/en-us/azure/machine-learning/service/how-to-manage-quotas).
#' @param workspace The `Workspace` object.
#' @param cluster_name A string of the name of the cluster.
#' @param vm_size A string of the size of agent VMs. More details can be found [here](https://aka.ms/azureml-vm-details). 
#' Note that not all sizes are available in all regions, as detailed in the 
#' aformentioned link. Defaults to `'Standard_NC6'`.
#' @param vm_priority A string of either `'dedicated'` or `'lowpriority'` to use 
#' either dedicated or low-priority VMs. Defaults to `'dedicated'`.
#' @param min_nodes An integer of the minimum number of nodes to use on the cluster. 
#' If not specified, will default to `0`.
#' @param max_nodes An integer of the maximum number of nodes to use on the cluster.
#' @param idle_seconds_before_scaledown An integer of the node idle time in seconds 
#' before scaling down the cluster. Defaults to `120`.
...
#' @param tags A named list of tags for the cluster, e.g. `list("tag" = "value")`.`
...
#' 
#' @description 
#' Provision an Azure Kubernetes Service instance (AksCompute) as a compute target 
#' for web service deployment. AksCompute is recommended for high-scale production 
#' deployments and provides fast response time and autoscaling of the deployed service. 
#' Cluster autoscaling isn't supported through the Azure ML R SDK. To change the nodes 
#' in the AksCompute cluster, use the UI for the cluster in the Azure portal. Once
#' created, the cluster can be reused for multiple deployments.
#' @param workspace The `Workspace` object.
#' @param cluster_name A string of the name of the cluster.
#' @param agent_count An integer of the number of agents (VMs) to host containers. 
#' Defaults to `3`.
#' @param vm_size A string of the size of agent VMs. More details can be found [here](https://aka.ms/azureml-vm-details). 
#' Note that not all sizes are available in all regions, as detailed in the 
#' aformentioned link. Defaults to `'Standard_D3_v2'`.
#' @param ssl_cname A string of a CName to use if enabling SSL validation on the 
#' cluster. Must provide all three - CName, cert file, and key file - to enable SSL
#' validation.
#' @param ssl_cert_pem_file A string of a file path to a file containing cert 
#' information for SSL validation. Must provide all three - CName, cert file, and 
#' key file - to enable SSL validation.
...
#' specified, defaults to the workspace location. Available regions for this compute can be
#' found here: "https://azure.microsoft.com/global-infrastructure/services/?regions=all&products=kubernetes-service".
#' @param vnet_resourcegroup_name A string of the name of the resource group where 
#' the virtual network is located.
#' @param vnet_name A string of the name of the virtual network.
#' @param subnet_name A string of the name of the subnet inside the vnet.
...
#' @export
#' @section Details:
#' For more information on using an AksCompute resource within a virtual network, see 
...
#' 
#' @description 
#' If you already have an AKS cluster in your Azure subscription, and it is version 
#' 1.12.##, you can attach it to your workspace to use for deployments. The existing 
#' AKS cluster can be in a different Azure region than your workspace.
#' 
#' If you want to secure your AKS cluster using an Azure Virtual Network, you must 
#' create the virtual network first. For more information, see 
...
#' @param resource_id A string of the resource ID for the AKS cluster being attached.
