...
#' @param max_duration_minutes An integer of the maximum duration of the HyperDrive run. 
#' Once this time is exceeded, any runs still executing are cancelled. If both 
#' `max_total_runs` and `max_duration_minutes` are specified, the hyperparameter tuning 
#' experiment terminates when the first of these two thresholds is reached.
#' @param policy The early termination policy to use. Can be either a `BanditPolicy`, 
#' `MedianStoppingPolicy`, or `TruncationSelectionPolicy` object. 
#' If `NULL` (the default), no early termination policy will be used. 
#' 
#' The `MedianStoppingPolicy` with `delay_evaluation of = 5` is a good termination policy 
#' to start with. These are conservative settings that can provide 25%-35% savings with no 
#' loss on primary metric (based on our evaluation data).
...
#' Assume that run X is the currently best performing run with an AUC (performance metric) 
#' of 0.8 after 100 intervals. Further, assume the best AUC reported for a run is Y. 
#' This policy compares the value `(Y + Y * 0.2)` to 0.8, and if smaller, cancels the run. 
#' If `delay_evaluation = 200`, then the first time the policy will be applied is at 
#' interval 200.
#' 
#' Now, consider a Bandit policy with `slack_amount = 0.2` and `evaluation_interval = 100`. 
#' If run 3 is the currently best performing run with an AUC (performance metric) of 0.8 
#' after 100 intervals, then any run with an AUC less than 0.6 (`0.8 - 0.2`) after 100 iterations 
#' will be terminated. Similarly, the `delay_evaluation` can also be used to delay the first 
#' termination policy evaluation for a specific number of sequences.
#' @section Examples:
#' In this example, the early termination policy is applied at every interval when metrics 
#' are reported, starting at evaluation interval 5. Any run whose best metric is less than 
...
#' corresponding (and earlier) performance of older runs for comparison. Therefore, 
#' runs aren't terminated for having a lower performance because they have run for 
#' less time than other runs. 
#' 
...
#' runs at interval 5.
#' ```
#' early_termination_policy = truncation_selection_policy(truncation_percentage = 20L,
#'                                                        evaluation_interval = 1L,
#'                                                        delay_evaluation = 5L)
#' ```
#' @md
...
#' @export
#' @section Details:
#' In this sampling algorithm, parameter values are chosen from a set of discrete 
#' values or a distribution over a continuous range. Functions you can use include: 
...
#' this variable is constrained to the interval `[exp(min_value), exp(max_value)]`.
...
#' is "smooth", and gets smoother with the size of the value, but which should be 
...
#' Specify a normal distribution of the form `round(exp(normal(mu, sigma)) / q) * q`.
#' 
#' Suitable for a discrete variable with respect to which the objective is smooth 
#' and gets smoother with the size of the variable, which is bounded from one side.
#' @param mu A double of the mean of the normal distribution.
#' @param sigma A double of the standard deviation of the normal distribution.
#' @param q An integer of the smoothing factor.
#' @return A list of the stochastic expression.
...
#' order of their best primary metric. If `reverse = TRUE`, the order is reversed. 
#' Each child in the result has run id, hyperparameters, best primary metric value, 
#' and status. 
#' 
