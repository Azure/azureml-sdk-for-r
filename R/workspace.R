...
#' workspace already exists or any of the workspace requirements are not satisfied.
#' When you create new workspace, it automatically creates several Azure resources 
#' that are used in the workspace:
#' 
#' * Azure Container Registry: Registers Docker containers that you use during
#'   training and when you deploy a model. To minimize costs, ACR is lazy-loaded
#'   until deployment images are created.
...
#' @param resource_group A string of the Azure resource group that is containing the
#' workspace. The parameter defaults to a mutation of the workspace name.
#' @param location A string of the location of the workspace. The parameter defaults 
#' to the resource group location. The location has to be a supported region for
#' Azure Machine Learning Services.
#' @param create_resource_group If `TRUE` the resource group will be created
#' if it doesn't exist.
#' @param friendly_name A string of the friendly name for the workspace that can be 
#' displayed in the UI.
#' @param storage_account A string of an existing storage account in the Azure 
#' resource ID format. The storage will be used by the workspace to save run outputs, 
#' code, logs etc. If `NULL` a new storage will be created.
#' @param key_vault A string of an existing key vault in the Azure resource ID 
#' format. The key vault will be used by the workspace to store credentials added to
#' the workspace by the users. If `NULL` a new key vault will be created.
#' @param app_insights A string of an existing Application Insights in the Azure 
#' resource ID format. The Application Insights will be used by the workspace to log
#' webservices events. If `NULL` a new Application Insights will be created.
#' @param container_registry A string of an existing container registry in the Azure
#' resource ID format. The container registry will be used by the workspace to pull 
#' and push both experimentation and webservices images. If `NULL` a new container 
...
#' ```
#' 
#' The following example shows how to reuse existing Azure resources by making use of 
#' all parameters utilizing the Azure resource ID format. The specific Azure resource 
#' IDs can be retrieved through the Azure Portal or SDK. This assumes that the resource 
#' group, storage account, key vault, App Insights and container registry already exist.
#' ```
#' ws <- create_workspace(name = 'myworkspace', 
#'                        subscription_id = '<azure-subscription-id>', 
#'                        resource_group = 'myresourcegroup', 
...
#' 
#' @description 
#' Returns a `Workspace` object for an existing Azure Machine Learning workspace. Throws an 
#' exception if the workpsace doesn't exist or the required fields don't lead to a uniquely 
#' identifiable workspace.
#' @param name A string of the workspace name to get.
#' @param subscription_id A string of the subscription ID to use. The parameter is required 
#' if the user has access to more than one subscription.
#' @param resource_group A string of the resource group to use. If `NULL` the method will 
...
#' 
#' @description 
#' Returns a `Workspace` object for an existing Azure Machine Learning workspace by 
#' reading the workspace configuration from a file. The method provides a simple way of 
#' reusing the same workspace across multiple files or projects. Users can save the 
