...
Lastly, use the `azureml` R library to install the compiled code. By default, `azuremlsdk::install_azureml()` will install the [latest version of the AzureML Python SDK](https://pypi.org/project/azureml-sdk/) in a conda environment called `r-azureml`.
...
``` {r Test installation, eval=FALSE}
library(azuremlsdk)
get_current_run()
```
